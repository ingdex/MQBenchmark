...
def do_gen_conf_kafka_producer(num_topics, message_size, async_enable):
    # 生成JSON数据
    json_data = []
...
def do_gen_conf_rocketmq_producer(num_topics, message_size, async_enable):
    # 生成JSON数据
    json_data = []
    filename = "rmq{}-1-{}".format(num_topics, message_size)
    # filename = "rmqConsumer-{}-1-{}".format(num_topics, message_size)
...
for n in numTopics:
    for s in messageSize:
        for sync in asyncEnable:
            do_gen_conf_kafka_producer(n, s, sync)
            do_gen_conf_rocketmq_producer(n, s, sync)